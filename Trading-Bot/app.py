...
class TradingBot:
    def __init__(self, settings):
        ...
        self.sl_pips = settings.get('sl_pips', 1000)
        self.tp_pips = settings.get('tp_pips', 1000)
        ...
        self.first_tp_pips = settings.get('first_tp_pips', 20)
        self.second_tp_pips = settings.get('second_tp_pips', 50)
        self.positive_sl_pips = settings.get('positive_sl_pips', 20)
        self.breakeven_trigger_pips = settings.get('breakeven_trigger_pips', 20)
        ...
        self.max_drawdown_points = settings.get('max_drawdown_points', self.sl_pips)
        
        # Trailing Stop Loss Strategy settings
        self.enable_trailing_sl = settings.get('enable_trailing_sl', False)
        self.trailing_breakeven_pips = settings.get('trailing_breakeven_pips', 30)
        self.trailing_positive_sl_pips = settings.get('trailing_positive_sl_pips', 30)
        self.trailing_positive_trigger_pips = settings.get('trailing_positive_trigger_pips', 60)
        self.trailing_tp_pips = settings.get('trailing_tp_pips', 90)
        self.trailing_sl_pips = settings.get('trailing_sl_pips', 30)
    ...
    def execute_trade_setup(self, is_buy):
        ...
        if self.strategy_type == 'exit_signal_or_max_tp':
            ...
            positions_to_open = 1  # Only one position for this strategy
            
            sl_price = current_price - (sl_pips * pip_value) if is_buy else current_price + (sl_pips * pip_value)
            tp_price = current_price + (tp_pips * pip_value) if is_buy else current_price - (tp_pips * pip_value)
            ...
            sl_pips = self.trailing_sl_pips
            tp_pips = self.trailing_tp_pips
            
            # Calculate checkpoint information
            checkpoint_size = tp_pips / self.dynamic_checkpoints
        ...
        try:
            ...
            for pos in positions:
                ...
                if self.strategy_type == 'exit_signal_or_max_tp':
                    ...
                    # Check if TP is hit
                    if pos.profit >= self.tp_pips * self.get_pip_value():
    ...
    def manage_drawdown_layers(self, positions):
        try:
            ...
            for setup_id, setup_positions in position_groups.items():
                ...
                drawdown_pips = drawdown / pip_value
                
                if drawdown_pips <= 0:
                    print(f"{self.symbol} Setup {setup_id} has no drawdown, no layers needed")
                    continue  # No drawdown, no need for additional layers
                
                print(f"{self.symbol} Setup {setup_id} drawdown: {drawdown_pips:.2f} pips")
                
                # Check if drawdown exceeds max limit - don't add layers if at max
                max_drawdown_pips = getattr(self, 'max_drawdown_points', self.sl_pips)
                if drawdown_pips >= max_drawdown_pips:
                    print(f"⚠️ WARNING: {self.symbol} Drawdown {drawdown_pips:.2f} pips exceeds max limit of {max_drawdown_pips} pips.")
                ...
                required_layers = math.ceil(drawdown_pips / self.drawdown_layer_threshold)
    ...
    def add_layer(self, setup_id, is_buy, layer_num):
        ...
        min_profit_pips = self.minimum_profit_per_position
        tp_price = current_price + (min_profit_pips * pip_value) if is_buy else current_price - (min_profit_pips * pip_value)
    ...
    def adjust_take_profits(self, positions):
        ...
        if buy_positions:
            ...
            pip_value = self.get_pip_value()
            min_profit_price = avg_entry + (self.minimum_profit_per_position * pip_value)
    ...
    def manage_trailing_sl(self, positions):
        ...
        for pos in positions:
            ...
            for i in range(1, self.dynamic_checkpoints + 1):
                ...
                if profit_pips >= checkpoint_level:
            ...
            if current_checkpoint == 1:
                ...
                progress_to_next = profit_pips - (checkpoint_size * (current_checkpoint - 1))
                percentage_of_current_checkpoint = progress_to_next / checkpoint_size
                
                lock_in_pips = (current_checkpoint - 1) * checkpoint_size * locked_profit_percentage
                ...
                if percentage_of_current_checkpoint > 0.3:  # Over 30% to next checkpoint
                    lock_in_pips += progress_to_next * locked_profit_percentage * 0.5
                    
                new_sl_pips = lock_in_pips
                new_sl = pos.price_open + (new_sl_pips * pip_value) if is_buy else pos.price_open - (new_sl_pips * pip_value)
            
            # Only update if new SL is better than current SL
            current_sl_diff = abs(pos.sl - pos.price_open) / pip_value
            if is_buy and (pos.sl == 0 or new_sl > pos.sl):
                print(f"{self.symbol} Moving SL to lock in {new_sl_pips:.2f} pips (profit: {profit_pips:.2f} pips, checkpoint: {current_checkpoint})")
    ...
    def run(self, initialize_mt5=True):
        ...
        print(f"  - SL/TP: {self.sl_pips}/{self.tp_pips} pips")
        print(f"  - Trade direction: {self.trade_direction}")
        
        if self.enable_progressive:
            print(f"Progressive Lockin Strategy enabled:")
            print(f"  - First TP: {self.first_tp_pips} pips")
            print(f"  - Second TP: {self.second_tp_pips} pips")
            print(f"  - Positive SL: {self.positive_sl_pips} pips")
            print(f"  - Breakeven trigger: {self.breakeven_trigger_pips} pips")
        
        if self.enable_drawdown_layering:
            print(f"Drawdown Layering Strategy enabled:")
            print(f"  - Threshold: {self.drawdown_layer_threshold} pips")
            print(f"  - Positions per layer: {self.positions_per_layer}")
            print(f"  - Minimum profit: {self.minimum_profit_per_position} pips")
        ...
        if self.enable_trailing_sl:
            ...
            print(f"  - Initial SL: {self.trailing_sl_pips} pips")
            print(f"  - Breakeven at: {self.trailing_breakeven_pips} pips profit")
            print(f"  - Positive SL at: {self.trailing_positive_sl_pips} pips profit when reaching {self.trailing_positive_trigger_pips} pips")
            print(f"  - TP: {self.trailing_tp_pips} pips")
...
def start_bot():
    ...
    base_settings = {
        ...
        'sl_pips': float(data.get('sl_pips', 100)),
        'tp_pips': float(data.get('tp_pips', 100)),
        'first_tp_pips': float(data.get('first_tp_pips', 20)),
        'second_tp_pips': float(data.get('second_tp_pips', 50)),
        ...
        'positive_sl_pips': float(data.get('positive_sl_pips', 20)),
        'breakeven_trigger_pips': float(data.get('breakeven_trigger_pips', 20)),
        ...
        'trailing_breakeven_pips': float(data.get('trailing_breakeven_pips', 30)),
        'trailing_positive_sl_pips': float(data.get('trailing_positive_sl_pips', 30)),
        'trailing_positive_trigger_pips': float(data.get('trailing_positive_trigger_pips', 60)),
        'trailing_tp_pips': float(data.get('trailing_tp_pips', 90)),
        'trailing_sl_pips': float(data.get('trailing_sl_pips', 30)),
...
def start_instance():
    """Start a single MT5 instance with the provided configuration"""
    data = request.json
    instance_id = data.get('instance_id')
    
    if instance_id is None:
        return jsonify({"status": "error", "message": "Instance ID is required"})
    
    # Add new settings for Prop Firm Mode
    instance_settings = {
        'instance_id': instance_id,
        'symbol': data.get('symbols', ['EURUSD'])[0],  # Primary symbol is the first one
        'symbols': data.get('symbols', ['EURUSD']),
        'timeframe': getattr(mt5, data.get('timeframe', 'TIMEFRAME_M15')),
        'lot_size': float(data.get('lot_size', 0.01)),
        'sl_pips': int(data.get('sl_points', 1000)),
        'tp_pips': int(data.get('tp_points', 1000)),
        'trade_direction': data.get('trade_direction', 'BOTH'),
        'login': data.get('login'),
        'password': data.get('password', ''),
        'server': data.get('server', ''),
        'magic_number': 234000 + int(instance_id),
        'pine_handler': pine_handler,
        'multi_symbol': len(data.get('symbols', ['EURUSD'])) > 1,
        'weekend_closing': data.get('weekend_closing', True),
        'max_drawdown': float(data.get('max_drawdown', 3.5)),
        'use_active_account': data.get('use_active_account', False),
        'enable_progressive': False,
        'enable_drawdown_layering': False,
        'enable_trailing_sl': False,
        'trailing_breakeven_pips': 30,
        'trailing_positive_sl_pips': 30,
        'trailing_positive_trigger_pips': 60,
        'trailing_tp_pips': 90,
        'trailing_sl_pips': 30,
        'dynamic_checkpoints': int(data.get('dynamic_checkpoints', 5)),
        ...
        'strategy': data.get('strategy', 'exit_signal_or_max_tp')
    }
    ...
    if strategy == 'progressive':
        ...
        instance_settings['first_tp_pips'] = int(tp_per_position)
        instance_settings['second_tp_pips'] = int(tp_per_position * 2)
        instance_settings['tp_pips'] = total_tp  # Keep the overall TP value
        
        # Standard settings
        instance_settings['positive_sl_pips'] = int(tp_per_position)  # Positive SL equal to first TP
        instance_settings['breakeven_trigger_pips'] = int(tp_per_position)  # Breakeven at first TP
        ...
        instance_settings['trailing_tp_pips'] = tp_value
        ...
        instance_settings['trailing_sl_pips'] = int(data.get('sl_points', 1000))  # Use the exact SL value provided by user
        instance_settings['trailing_breakeven_pips'] = int(checkpoint_size)  # First checkpoint
        instance_settings['trailing_positive_sl_pips'] = int(checkpoint_size * 0.5)  # Half of a checkpoint
        instance_settings['trailing_positive_trigger_pips'] = int(checkpoint_size * 2)  # At second checkpoint
        ...
        instance_settings['first_tp_pips'] = int(tp_per_position)
        instance_settings['second_tp_pips'] = int(tp_per_position * 2)
        instance_settings['tp_pips'] = total_tp
        instance_settings['positive_sl_pips'] = int(tp_per_position)
        instance_settings['breakeven_trigger_pips'] = int(tp_per_position)
