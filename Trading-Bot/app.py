...
class TradingBot:
    ...
    def should_open_trade(self):
        ...
        # Check prop firm mode restrictions only if enabled
        ...
        if self.strategy_type == 'exit_signal_or_max_tp':
            # Check existing positions for the current symbol
            ...
            if existing_positions:
                ...
                # Check if we've reached the maximum number of positions allowed for this symbol
                if existing_count >= self.reentry_count:
                    print(f"Maximum number of positions ({self.reentry_count}) reached for {self.symbol}")
                    return False
                
                # Check if any position for this symbol is in drawdown (negative profit)
                ...
                print(f"Found {len(positions_in_drawdown)} positions in drawdown for {self.symbol}, allowing reentry ({existing_count}/{self.reentry_count})")
                return True
            else:
                # If there are no existing positions for this symbol, we can open the first one without checking drawdown
